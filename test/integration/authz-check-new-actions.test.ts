...
describe.skipIf(process.env.RUN_INTEGRATION_TESTS !== "true")(
  ...
  test("seed inserts new permissions", async () => {
    const keys = [...updateActions, ...readOnlyListActions, ...adminListActions];
  ...
  test("seed maps new permissions to roles", async () => {
    ...
    const permRows = await db
      .select()
      .from(permissions)
      .where(inArray(permissions.key, [...updateActions, ...readOnlyListActions, ...adminListActions]));
    ...
    for (const key of [...updateActions, ...readOnlyListActions, ...adminListActions]) {
      expect(hasMapping("workspace_owner", key)).toBe(true);
      expect(hasMapping("content_editor", key)).toBe(true);
    }
    for (const key of readOnlyListActions) expect(hasMapping("read_only", key)).toBe(true);
    for (const key of adminListActions) expect(hasMapping("read_only", key)).toBe(false);
  ...
  test("owner/editor allowed for update actions", async () => {
    for (const actionKey of updateActions) {
      ...
      expect(((await ownerRes.json()) as CheckSuccess).data.allowed).toBe(true);
      ...
      expect(((await editorRes.json()) as CheckSuccess).data.allowed).toBe(true);
    }
  }, 15_000);
